...
def update_bq_data():
    ...
    protocolsRegistered.to_csv(f'{DATA_DIR}/protocolsRegistered.csv', index=False)
    ...
    # get rewards 
    rewards = bq_query_get(QUERY_DICT['RewardDistribution'])
...
def read_bq_data_from_csv():
    ...
    deposits = pd.read_csv(f"{DATA_DIR}/deposits.csv", dtype={'nAmount': float, 'nFee': float})
   
    withdrawls = pd.read_csv(f"{DATA_DIR}/withdrawls.csv",dtype={'nAmount': float, 'nFee': float} )
